...
export default function InfoHabilidades() {
  ...
  const handleSave = () => {
    ...
    storedData.push(...skills);
    localStorage.setItem("skillData", JSON.stringify(storedData));
    console.log("Informacion Habilidades:", storedData);
    ...
    <div className="flex">
      ...
      <div className="flex flex-col justify-center items-center w-auto md:w-screen lg:w-4/5">
        <div className="border-b-4 border-[#006BA6] flex-row flex items-center justify-center p-2 md:w-full lg:m-2 lg:w-full">
          <h2 className="m-4 ml-14 md:ml-12 lg:m-5 text-center text-xl md:text-3xl lg:text-4xl text-black font-bold">HABILIDADES</h2>
          <div
          ...
          {skills.map(
            (skill, index) =>
              index === skills.length - 1 && (
                <form
                className="info-habilidades justify-center items-center rounded-md text-white bg-[#006BA6] flex flex-col md:gap-4 gap-0 lg:gap-4 mt-12 py-4 md:px-12 lg:py-8 lg:px-24 w-auto h-auto"
                  key={index}
                >
